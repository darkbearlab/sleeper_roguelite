...
- **地圖編輯器 `editor.html`**（與 index 分檔）：四分頁(地圖物件/waypoint+連線/敵人配置/底圖)、拖曳、連線編輯、邊穿牆紅標、特徵庫 `FEATURES`、框選群組(transient、無持久 `_gid`)、門切牆缺口、參考底圖(可旋轉，editor-only)、地圖尺寸可調、新增 waypoint 自動連線。**工具列分群**(地圖庫/本圖/檢視/單張，`.grp`+群標+tooltip)+「？」說明(`EDITOR_HELP` 開 overlay)。**比例尺**：地圖左下角 1 公尺×1 公尺方框(`PX_PER_M=40`、`drawScaleRef`、世界空間隨縮放)。**多地圖庫 maps.json**(`{鍵:地圖}`)：讀入整份/下拉切換/＋新增/🗑刪除/改鍵/下載；model 層 `mapLib/curKey/syncCurrentToLib/selectMap/addMap/deleteMap/libImportObj/libExportObj`。**踩雷：刪 waypoint 必須重編號**（遊戲 `wp=id=>MAP.waypoints[id]` 以陣列索引當 id，連帶 edges/enemySpawns.nodeId）。editor.html 自帶一份小常數(ENEMY_TYPE_IDS/ENEMY_META/WP_TAGS/DESTRUCT_PRESETS/FEATURES/TEMPLATES)，改 index 對應內容要手動同步。in-index 編輯器(scene EDITOR)仍並存。
...
MapDef   { label,width,height,walls,doors,lowWalls,bushes,destructibles,buttons,waypoints,edges,
...
armor{blockChance0.85} flank{flankMult2.0,surpriseMult1.5} lowWall{slow0.3,touchR12,closeRange90,blockChance0.55}
...
- **地圖重用：鏡像翻轉 + 隨機進/撤點**（2026-06-24 討論，**決定先不做**）。若日後做，建議：①**鏡像優先**＝runtime 剛性座標變換(H:`x'=W−x`、V:`y'=H−y`，敵朝向 H:`π−θ`/V:`−θ`/HV:`θ+π`，edges 索引不變、nextHop/巡邏自動重算)，**零平衡風險、所有現有圖白賺 4 變體、免重做**(排除 intro)。②**隨機進/撤點**次之＝`entryPoints[]`/`extractionPoints[]` 本就是陣列(目前固定取[0])，每場挑一組；nextHop/巡邏/追兵點自動適配；**踩雷**：現有圖哨兵朝向針對「左進右撤」寫死→需「預設朝向的哨兵開場自動轉向 ENTRY」+ 做成 opt-in(圖宣告多候選點才啟用)。③隨機**per-run 種子化**(綁路線圖節點，一個 run 內固定可學圖)。更強的 replay 槓桿其實是**敵人配置隨機**(抽 spawn 子集/類型/數量)，更大工程、另議。
