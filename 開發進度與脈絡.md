...
- **地圖編輯器 `editor.html`**（與 index 分檔）：三分頁(地圖物件/waypoint+連線/敵人配置)、拖曳、連線編輯、邊穿牆紅標、特徵庫 `FEATURES`、框選群組(transient、無持久 `_gid`)、門切牆缺口、參考底圖(可旋轉，editor-only)、地圖尺寸可調、新增 waypoint 自動連線。**多地圖庫 maps.json**(`{鍵:地圖}`)：讀入整份/下拉切換/＋新增/🗑刪除/改鍵/下載；model 層 `mapLib/curKey/syncCurrentToLib/selectMap/addMap/deleteMap/libImportObj/libExportObj`。**踩雷：刪 waypoint 必須重編號**（遊戲 `wp=id=>MAP.waypoints[id]` 以陣列索引當 id，連帶 edges/enemySpawns.nodeId）。editor.html 自帶一份小常數(ENEMY_TYPE_IDS/ENEMY_META/WP_TAGS/DESTRUCT_PRESETS/FEATURES/TEMPLATES)，改 index 對應內容要手動同步。in-index 編輯器(scene EDITOR)仍並存。
