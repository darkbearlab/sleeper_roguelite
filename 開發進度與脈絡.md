...
- **搜刮獎勵池重設計**（2026-06-25 規劃中）：rest scavenge 改成「乾淨的裝備抽取」＝rarity 加權 + targeted 單人強化(取代平板全隊+HP/彈)；風險/賭注/build 歪移到事件。
- **隨機事件節點**（2026-06-25 規劃中，下一步）：新節點型別 `EVENT`(scene EVENT)＝資料驅動 `EVENTS` 目錄(title/text/choices[{label,outcome}])，進節點隨機抽一個(避重)、選項套 outcome → 前進。outcome 用現有資源(HP/彈/裝備/單位)無新貨幣：補給/治療/±maxHP(全隊或單人)/拿武裝備技道具入倉/**off-tier 跨樹被動(歪掉 relic)**/招募/風險(機率好壞)/取捨(付代價換好物)。沿用 SETTLE/REST 的 canvas scene + 點選卡片 + needTarget 子階。
