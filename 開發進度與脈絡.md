...
- **cutscene 對話「選項」節拍**：beat `{kind:'choice', text?, pos?, options:[{label, fn?, set?, queue?, then?:[beats]}]}`。`beatEnter`(choice)＝列選項、鎖;**按數字鍵 1~9 選**(演出鎖滑鼠故用鍵盤;mousedown/keydown 都攔到 choice 分支)。`beatDone`(choice)＝選後套 opt.fn/set(旗標)/queue(後續)，把 `opt.then` 節拍 `splice` 到後面續播＝分支對話。`drawCutscene` 在畫面下方畫 `[n] 選項`。＝所有劇情演出都能分歧(序章 + 未來 scene 事件共用)。
