...
## 2. 場景與流程
`scene ∈ { MENU, MAP, COMBAT, REST, SETTLE, ARMORY, RUNEND, RUNWIN, EDITOR, SANDBOX }`
...
- **休息關 meta**：`warehouse`(有上限=人數×slotsPerMember)/`backpack`/`ammoStock`；搜刮三選一(地板保證)/喚醒；配裝卡(個性+武器+技能 SK+裝備🛡+道具+彈藥±+📁機密檔案指派+**↻轉職** `cycleArchetype`)；`drawRest`/`restRects`/`restClick`。
...
```
RosterEntry { name,hue,strength,weaponId,abilityId?,archetypeId,tier(0~3),passives[],bonusMaxHp,hp,allocatedAmmo,
...
- **✅ 成長弧（個性技能樹 + 戰果結算升階）已實作**（2026-06-25，4 批；headless 全綠）＝詳見 §4「成長弧」。解 STS 死亡螺旋/無 build。仍待續的子題：
  - **雙武器**：主+副兩把；換槍時間隨 weight(同負重影響移動的邏輯)；swap 可當「比換彈快的應急」＝取捨;佔 loadout 槽/負重＝與多帶 gadget 的機會成本;idle/本能單位先只用主武器(換槍是被附身單位的戰術工具)。pistol 底線可當可靠副武器。
  - **被動樹數值平衡**：18 條都是佔位數(`CONFIG.passive`)，要實際試玩調。fog 視錐沒吃 scout 的寬錐(只 canSee 吃)＝小限制。
  - **新隊員加入機制**：成長弧目前只「變強」沒「補人」(隊員仍只減不增,只有休息喚醒)；可考慮戰後/事件給新兵。
- **隨機事件節點**：路線圖加 事件/商店/風險 節點(STS 式取捨)，深化「這局怎麼走」；**事件可直接塞技能**(off-tier/跨樹/超曲線)＝windfall「歪掉」relic 時刻。
